...
- ESPHome firmware entry file: `namo.yaml`
- Shared ESPHome package: `packages/namo/`
...
NAMO v0.5.5 is currently a public beta.

PIR and confirmed LD2450 tracking can start a presence session, while the LD2450 also provides target position, room, zone, and exit evidence. New hardware can attach an optional LD2410C to maintain an already established session when the LD2450 loses a motionless person. The LD2410C cannot start presence by itself or create motion and spatial tracking results. NAMO is designed to retain presence more reliably than motion-only sensors, but perfectly motionless presence detection is not guaranteed in every room or installation. Detection performance depends on placement, range, orientation, and the surrounding environment.

LD2410C assistance arms only after its detection overlaps PIR or confirmed LD2450 presence for 2 seconds. Confirmed exit evidence or filter blocking vetoes the assistance. If stable presence ends without either veto, an armed LD2410C may reassert the same session for up to 30 seconds. Its detection distance is stabilized with a three-sample median for held-target display. Missing or unavailable LD2410C hardware is ignored, leaving the existing PIR and LD2450 behavior unchanged.
...
1. Download `namo-v<version>-factory.bin` from GitHub Releases.
...
NAMO requires a Seeed Studio XIAO ESP32S3 with 8MB Flash / 8MB PSRAM and an LD2450-series radar. The LD2410C presence-assist radar, PIR, light, and environmental sensors are optional.
...
```powershell
esphome config namo.yaml
esphome compile namo.yaml
```
...
The normal `npm test` command runs setup-page i18n checks, replay parser tests,
and dashboard tests. Native PresenceTracker tests are separate because they
require `cl`, `g++`, or `clang++`; run `npm run test:native` explicitly. That
command fails with a clear message when no supported host compiler is available.
...
When an LD2410C is available, the dashboard also shows whether the auxiliary presence sensor currently detects a target.
...
From the Zone Settings radar map, an available LD2410C can be tuned by distance band. Per-gate engineering telemetry is enabled only while the tuning panel is open and is disabled again when the panel closes.
...
- [Exit evidence](docs/presence-exit-zone.md)
...
- ESPHome 펌웨어 진입 파일: `namo.yaml`
- ESPHome 공통 패키지: `packages/namo/`
...
<!-- release-firmware-version -->
NAMO v0.5.5는 현재 공개 베타 단계입니다.

PIR과 확정된 LD2450 트랙은 재실 세션을 시작할 수 있으며, LD2450은 타깃 위치, 방, 구역 및 퇴실 근거도 제공합니다. 신규 하드웨어에서는 선택 사항인 LD2410C가 LD2450이 움직이지 않는 사람을 놓쳤을 때 이미 시작된 재실을 유지할 수 있습니다. LD2410C 단독으로 재실을 새로 켜거나 움직임 및 공간 추적 결과를 만들지는 않습니다. NAMO는 움직임만 감지하는 센서보다 재실 상태를 안정적으로 유지하도록 설계되었지만, 모든 방과 설치 환경에서 완전히 움직이지 않는 사람의 감지를 보장하지는 않습니다. 감지 성능은 센서 배치, 거리, 방향 및 주변 환경의 영향을 받습니다.

LD2410C 보조는 PIR 또는 확정된 LD2450 재실과 2초 동안 함께 감지된 뒤에만 준비됩니다. 확인된 퇴실 근거나 필터 차단이 있으면 보조를 사용하지 않습니다. 두 차단 근거 없이 안정화 재실이 종료되면 준비된 LD2410C가 최대 30초 동안 같은 재실 세션을 다시 이어갈 수 있습니다. 보존 타깃 표시에 사용하는 감지 거리는 3개 표본의 중앙값으로 안정화합니다. LD2410C가 없거나 통신할 수 없으면 조용히 무시하고 기존 PIR 및 LD2450 동작을 그대로 유지합니다.
...
1. GitHub Releases에서 `namo-v<version>-factory.bin`을 받습니다.
...
일반 `npm test`는 setup page i18n 검사, 리플레이 파서 테스트와 대시보드 테스트를
실행합니다. 네이티브 PresenceTracker 테스트는 `cl`, `g++`, `clang++` 중 하나가
필요하므로 `npm run test:native`로 별도 실행합니다. 지원하는 호스트 컴파일러가
없으면 이 명령은 원인을 명확히 표시하고 실패합니다.
...
LD2410C가 연결되어 있으면 보조 재실 센서가 현재 타깃을 감지하는지도 함께 표시합니다.
...
구역 설정의 레이더맵에서는 연결된 LD2410C의 거리 띠별 민감도를 조절할 수 있습니다. 게이트별 engineering 계측은 튜닝 패널을 연 동안에만 활성화되고 패널을 닫으면 다시 비활성화됩니다.
...
- [퇴실 근거](docs/presence-exit-zone.ko.md)
