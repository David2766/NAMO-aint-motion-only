# NAMO 하드웨어

[English](README.md)

이 폴더에는 NAMO의 기준 하드웨어 정보와 제작 파일이 들어 있습니다.

> [!IMPORTANT]
> 현재 듀얼 레이더 PCB 제작 파일과 회로도는 [`pcb/dual_sensor/`](pcb/dual_sensor/)에 공개되어 있습니다. 케이스 파일과 완성품 사진은 아직 구형 단일 레이더 세대의 자료입니다. 듀얼 레이더용 케이스는 계속 다듬는 중이며 아직 공개하지 않았으므로, 현재 듀얼 레이더 보드에 구형 케이스를 그대로 출력하지 마세요.

## 제작 사진

| 1. PCB | 2. BH1750FVI 조도 센서 |
| --- | --- |
| ![NAMO PCB](../docs/img/hardware/1.jpg) | ![NAMO BH1750FVI 조도 센서](../docs/img/hardware/2.jpg) |
| 3. FPC-SHT40 온습도 센서 | 4. LD2450 레이더 |
| ![NAMO FPC-SHT40 온습도 센서](../docs/img/hardware/3.jpg) | ![NAMO LD2450 레이더](../docs/img/hardware/4.jpg) |
| 4-1. LD2410C 정적 재실 레이더 | 5. 조립 완료 후 후면을 닫기 전 상태 |
| ![NAMO LD2410C 정적 재실 레이더](../docs/img/hardware/4-1.jpg) | ![NAMO 조립 완료 후 후면을 닫기 전 상태](../docs/img/hardware/5.jpg) |
| 6. 최종 조립 상태 | |
| ![NAMO 최종 조립 상태](../docs/img/hardware/6.jpg) | |

1-6번 사진은 구형 단일 레이더 조립 구성을 보여줍니다. 4-1번은 현재 듀얼 레이더 설계에서 선택적으로 사용하는 LD2410C입니다. 듀얼 레이더용 케이스와 최종 조립 사진은 케이스 설계가 확정된 뒤 추가할 예정입니다.

## 지원 하드웨어

| 하드웨어 | 필수 여부 | 설명 |
| --- | ---: | --- |
| Seeed Studio XIAO ESP32S3, 8MB Flash / 8MB PSRAM | 필수 | 현재 파티션 및 메모리 구성에 필요 |
| LD2450 계열 mmWave 레이더 | 필수 | 재실 시작 및 공간 추적 센서 |
| LD2410C 정적 재실 레이더 | 선택 | 사용 가능할 때 이미 시작된 재실 세션을 유지 |
| Panasonic EKMC1603111 PIR 센서 | 선택 | GPIO1 사용; 생략할 경우 입력이 전기적으로 안정되어야 함 |
| BH1750 조도 센서 | 선택 | I2C 버스 공유 |
| SHT4x 온습도 센서 | 선택 | I2C 버스 공유 |

이 펌웨어는 8MB Flash / 8MB PSRAM 모델과 커스텀 파티션 구성을 기준으로 합니다. Flash 용량이 작거나 PSRAM이 없는 ESP32-S3 보드, 또는 ESP32-C6 같은 다른 칩 계열에서는 그대로 동작하지 않을 수 있습니다.

## 핀 연결

기준 펌웨어와 PCB는 Seeed Studio XIAO ESP32S3의 아래 핀 배열을 사용합니다.

| 기능 | XIAO 표기 | ESP32 GPIO | 연결 대상 |
| --- | --- | --- | --- |
| PIR 입력 | D0 | GPIO1 | PIR 센서 OUT |
| I2C SDA | D4 | GPIO5 | BH1750 SDA, SHT4x SDA |
| I2C SCL | D5 | GPIO6 | BH1750 SCL, SHT4x SCL |
| LD2410C UART TX | D1 | GPIO2 | LD2410C RX |
| LD2410C UART RX | D2 | GPIO3 | LD2410C TX |
| LD2450 UART TX | D6 | GPIO43 | LD2450 RX |
| LD2450 UART RX | D7 | GPIO44 | LD2450 TX |
| 상태 LED | 내장 LED | GPIO21 | XIAO 내장 사용자 LED |
| 초기화 버튼 | BOOT | GPIO0 | XIAO BOOT 버튼 |

## 배선 주의사항

LD2450과 LD2410C의 TX/RX는 각각 XIAO UART와 교차 연결합니다. XIAO의 TX는 레이더 RX로, XIAO의 RX는 레이더 TX로 연결합니다.

GPIO3은 ESP32-S3의 strapping pin입니다. 기준 보드는 이 핀을 LD2410C TX에 연결하므로, 부팅 strap level을 강제로 바꿀 수 있는 외부 pull-up 또는 pull-down 저항을 추가하지 마세요.

BH1750과 SHT4x는 같은 I2C 버스를 공유합니다. 각 모듈의 동작 전압을 확인한 뒤 전원을 연결하고, 모든 모듈의 GND를 XIAO의 GND와 공통으로 묶어야 합니다.

LD2450은 재실, 타깃 추적 및 구역 감지에 반드시 필요합니다. LD2450이 없어도 펌웨어가 부팅될 수는 있지만 해당 기능은 정상적으로 동작하지 않습니다.

LD2410C는 런타임 선택 사항입니다. 장착된 경우 PIR 또는 확정된 LD2450 트랙이 이미 시작한 재실 세션을 유지할 수 있습니다. LD2410C 단독으로 재실을 새로 켜지 않으며 움직임, 타깃 좌표와 개수, 방 상태 또는 구역 재실을 만들지 않습니다. raw 재실, 이동/정지 거리와 energy는 진단에 유지되며, replay에는 안정화된 기준 감지 거리와 보조 상태도 기록됩니다.

보조 기능은 LD2410C 감지가 PIR 또는 확정된 LD2450 재실과 2초 동안 겹친 뒤에만 준비됩니다. 확인된 퇴실 근거와 필터 차단은 보조를 막습니다. 두 차단 근거가 없으면 준비된 LD2410C가 최근 종료된 동일 재실 세션에 대해 30초 재진입 시간을 가집니다. 기준 감지 거리는 3개 표본의 중앙값을 사용합니다. 하드웨어가 없으면 조용히 무시하며, 대시보드 튜닝 세션은 패널을 연 동안에만 게이트별 engineering 계측을 활성화합니다.

BH1750, SHT4x 및 PIR 센서는 생략할 수 있습니다. 이 경우 해당 센서 값이 표시되지 않고 로그에 경고가 남을 수 있습니다. PIR 센서를 생략한다면 GPIO1 입력이 떠서 오탐이 발생하지 않도록 회로에서 안정적으로 처리하거나 YAML의 PIR 관련 설정을 제거하세요.

## PCB 제작 파일

현재 듀얼 레이더 설계에는 위 핀맵에 설명한 선택 사항인 LD2410C UART 연결이 추가되어 있습니다. 이전 단일 레이더 보드는 레거시 참고 자료로 함께 유지합니다.

- 현재 듀얼 레이더 보드: [Gerber 압축 파일](pcb/dual_sensor/Gerber.zip) · [회로도](pcb/dual_sensor/schematic.png)
- 구형 단일 레이더 보드: [Gerber 압축 파일](pcb/single_sensor/Gerber.zip) · [회로도](pcb/single_sensor/schematic.png)

주문하기 전에 선택한 제작 압축 파일, 회로도, 전원 전압, 커넥터 방향 및 보드 요구사항을 직접 확인하세요. 현재 LD2450 + LD2410C 기준 하드웨어를 제작할 때는 듀얼 레이더 파일을 사용해야 합니다.

## 케이스 모델

아래 케이스 파일은 구형 단일 레이더 하드웨어 세대에 맞춘 모델입니다. 듀얼 레이더 배치용 신형 케이스는 현재 계속 다듬는 중이며 아직 공개하지 않았습니다.

- [상단 케이스](enclosure/top.3mf)
- [하단 케이스](enclosure/bottom.3mf)

3MF 파일을 슬라이서에서 열고 크기, 출력 방향, 서포트 및 재료 설정을 확인한 뒤 출력하세요. 프린터 보정 상태와 재료 수축에 따라 최종 결합 상태가 달라질 수 있습니다.

구형 케이스 모델은 수정 없이 현재 듀얼 레이더 PCB와 센서 배치에 맞지 않을 수 있습니다.

## 자작 구성 참고

동일한 XIAO ESP32S3 보드와 위 핀맵을 사용하면 호환 기기를 직접 제작할 수 있습니다. 전원을 넣기 전에 각 모듈의 전원 전압과 핀 방향을 반드시 확인하세요.

PCB와 케이스 파일은 위에 명시된 펌웨어 핀맵을 기준으로 사용해야 합니다. 회로나 커넥터 배치가 바뀌면 펌웨어 설정과 이 문서를 함께 갱신해야 합니다.

## 라이선스

Copyright (c) 2026 David2766.

이 디렉터리의 하드웨어 설계 및 제작 파일은 [Creative Commons 저작자표시-비영리-동일조건변경허락 4.0 국제 라이선스](LICENSE)(`CC BY-NC-SA 4.0`)로 배포됩니다. 상업적 사용에는 프로젝트 제작자의 별도 허가가 필요합니다.
